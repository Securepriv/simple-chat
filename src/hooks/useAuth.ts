...
export function useAuth() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (session?.user) {
        ...
        if (profile) {
          ...
          await supabase.from('users').update({ status: 'online' }).eq('id', session.user.id)
        }
      }
      setLoading(false)
    }
    ...
    const handleUnload = async () => {
      ...
      if (u) await supabase.from('users').update({ status: 'offline', last_seen: new Date().toISOString() }).eq('id', u.id)
    }
  ...
  const register = async (email: string, password: string, username: string) => {
    const { error } = await supabase.auth.signUp({
      email, password,
      options: { data: { username }, emailRedirectTo: `${process.env.NEXT_PUBLIC_APP_URL}/auth/callback` },
    })
    if (error) throw error
  }
